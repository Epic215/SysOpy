...
int main(int argc, char* argv[]) {
    ...
    //connect serwer
    sfd_c = socket(AF_INET,SOCK_STREAM,0);
    if (sfd_c== -1) {
        perror("socket");
    }
    ...
    if (connect(sfd_c, (struct sockaddr *) &addr, sizeof(struct sockaddr_in)) == -1){
        perror("connect");
    }

    write(sfd_c,client_id, strlen(client_id));
